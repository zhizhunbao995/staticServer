...
Magic.prototype.use = function(route,middleware) {
    ...
    this.middlewares.push({path:path,handle:handle})
}
...
Magic.prototype.listener = function(req, res) {
    ...
    function nextFilter(){
        ...
        if (self.middlewares[req[curKey]]) {
            self.middlewares[req[curKey]]["handle"](req, res, (err) => {
                if (err) {
...
function walk(path){  
    ...
    dirList.forEach(function(item){
        var floder = "",
            fileName = PATH.normalize((path +"/"+ item).replace(workUrl,""));
        if (FS.statSync(path + '/' + item).isDirectory()) {
            folder.push({url:fileName,file:item,floder:"floder"})
        }
        if (item.indexOf("\.") != 0) {//隐藏文件不展示
            arrPath.push({url:fileName,file:item,floder:floder})
        }
    });

    return folder.concat(arrPath);
}
